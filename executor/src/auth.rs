...
pub async fn add_api_key(key: String, role: Role, rate_limit: u32) {
    ...
    // Persist to DB so keys survive restarts
    if let Some(db) = crate::workflow_registry::get_db() {
        let role_str = format!("{:?}", role);
        let _ = db.save_api_key(&key, &role_str, rate_limit, created_at);
    }
}
...
/// Load API keys persisted in the database into the in-memory map.
/// Call once during server startup, after `init_api_keys`.
pub async fn load_keys_from_db() {
    if let Some(db) = crate::workflow_registry::get_db() {
        if let Ok(rows) = db.load_api_keys() {
            let mut map = API_KEYS.write().await;
            for (key, role_str, rate_limit, created_at) in rows {
                // Don't overwrite keys already set by init_api_keys
