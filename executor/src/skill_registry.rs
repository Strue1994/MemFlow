...
impl AsyncSkillRegistry {
    ...
    pub async fn add_skill(&self, skill: Skill) -> Result<(), String> {
        self.registry.write().await.register(skill);
        Ok(())
    }

    pub async fn find_skills(&self, query: &str) -> Vec<Skill> {
        self.registry.read().await.search(query).into_iter().cloned().collect()
    }

    pub async fn list_all(&self) -> Vec<Skill> {
        self.registry.read().await.list_all().into_iter().cloned().collect()
    }

    pub async fn reload_from_directory(&self, dir_path: &str) -> Result<u32, String> {
        use std::fs;
        use std::path::Path;
...
mod tests {
    ...
    fn test_find_matching() {
        let matches = find_matching_skills("fetch");
